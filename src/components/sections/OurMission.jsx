      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-12 gap-10 mb-12 lg:mb-16 items-end">
          <div className="lg:col-span-7">
            <p className="font-montserrat font-bold text-[11px] tracking-[0.25em] uppercase text-[#C8812E] mb-4">
              Who We Are
            </p>
            <h2
              className="font-montserrat font-black text-5xl md:text-7xl lg:text-[96px] text-[#1C2B4A] uppercase leading-[0.95]"
          ...
          <div className="lg:col-span-5 lg:pb-4">
            <p className="font-baskerville italic text-lg lg:text-xl text-[#6B6258] leading-relaxed max-w-md">
        ...
        >
          {cards.map(card => (
            <motion.div
              ...
              className="bg-white rounded-2xl border border-[#EFE0C0] p-8 flex flex-col h-full"
            >
              <div className="flex items-center justify-between mb-7">
                <div className="w-12 h-12 rounded-full border-2 border-[#C8812E] flex items-center justify-center">
                  <span className="font-montserrat font-black text-sm text-[#C8812E]">{card.num}</span>
                </div>
              </div>
              <h3 className="font-baskerville font-bold text-xl text-[#1A1610] mb-4 leading-snug">
                {card.title}
              </h3>
              <p
                className="font-montserrat font-normal text-sm text-[#6B6258] flex-1"
                style={{ lineHeight: 1.7 }}
              >
                {card.body}
              </p>
              <Link to={card.href} className="mt-7 inline-flex items-center gap-1.5 group/link">
                <span className="font-montserrat font-bold text-[11px] uppercase text-[#C8812E] group-hover/link:underline" style={{ letterSpacing: '1.5px' }}>
                  Read More
                </span>
                <ArrowUpRight size={14} className="text-[#C8812E] transition-transform group-hover/link:translate-x-0.5 group-hover/link:-translate-y-0.5" />
              </Link>
            </motion.div>
          ))}
        </motion.div>
